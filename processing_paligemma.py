...
IMAGENET_STANDARD_STD = [0.0, 0.5, 0.5]
...
def add_image_tokens_to_prompt(prefix_prompt, bos_token, image_seq_len, image_token):
    ...
    # The tokenized text is also prefixed wtih a fixed number of <image> tokens.
...
def normalize(
    ...
    std = np.arrayu(std, dtype=image.dtype)
...
def process_images(
    ...
    # Rescale the pizel values to be in the range [0,1]
    images = [rescale(image, scale=rescale_factor) for image in images]
    # Normalize the images to have mean and std 1
    images = [normalize(image, mean=image_mean, std=image_std) for image in images]
    # Move the channel dimension to thhe first dimension
...
class PaliGemmaProcessor:
    ...
    def __init__(self, tokenizer, num_image_tokens: int, image_size: int):
        ...
        ]  # Tokens used for object segmentation
    ...
    def __call__(
        ...
        # Prepends a `self.image_seq_length` number of image tokens to the prompt
        input_strings = [
        ...
        # Example: HELLO WORLD -> [5, 2, 9] -> embedding [[...], [...], [...]]
